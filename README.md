...
> CPU不再接着（刚执行完的指令）向下执行，而是转去处理中断信息
>
> 内中断：CPU内部发生的事件引起的中断
>
> * 除法中断（0)
...
>
> 使用t命令，Debug将TF标志设置为1，使CPU工作在单步中断方式下
...
> eg: 执行完向ss寄存器传送数据的指令后，即便发生中断，CPU也不会响应
>
> mov ax, 1000
>
> 1. mov ss, ax
...
> 0~255以内的端口读写，端口号用立即数给出
>
> 256~65535内，端口号放在dx中

> 在in和out指令中，**只能**使用 **ax** 或 **al** 来存放读入和写出的数据。
>
> 访问8位端口使用al，访问16位端口时用ax

## CMOS RAM

> 1. 包含一个实时钟和一个有128个存储单元的RAM存储器
> 2. 128个字节的RAM中存储：内部实时钟，系统配置信息相关的程序
> 3. CMOS RAM靠电池供电
> 4. 读取CMOS步骤
>    1. 将要读取的单元地址送入70H地址端口
>    2. 从数据端口71H读出指定单元的内容

## bios的int9和int16h中断例程

> BIOS的int9中断例程和int16h中断例程是一对相互配合的程序，int9中断例程向键盘缓冲区写入，int16h中断例程从缓冲区中读出

> 它们写入和读出的时机不同，int9中断例程在有键按下的时候向键盘缓冲区写入数据，而int16h中断例程是在应用程序对其进行调用时候，将数据从键盘缓冲区中读出。
...
> BIOS提供的int 16h中断。当(AH)=0时，从键盘缓冲区读取一个键盘输入，并且将其从缓冲区中删除
